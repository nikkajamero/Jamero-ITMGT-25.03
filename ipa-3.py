...
# Calling the function to see if the code works.
from_member = "@jobenilagan"
to_member = "@joaquin"
...
def tic_tac_toe(board):
    ...
    # Replace `pass` with your code.
    # Stay within the function. Only use the parameters as input. The function should return your answer.
    def checkboard_win(symbol, size):
        for i in range(size):
    ...
    if checkboard_win('X', size):
        return 'X'
    elif checkboard_win('O', size):
        return 'O'
    else:
        return "NO WINNER"
...
# Calling the function to see if it is working.
tic_tac_toe(board7)
...
def eta(first_stop, second_stop, route_map):
    ...
    while True:
        next_stops = [i[1] for i in route_map if i[0] == current_stop]
        if not next_stops:
            return -1
